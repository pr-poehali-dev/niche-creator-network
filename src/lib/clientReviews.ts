// ВНИМАНИЕ: это НЕ отзывы реальных клиентов.
// Это составленные платформой ПРИМЕРЫ типичных обращений — иллюстрация того,
// с какими задачами приходят к специалистам. На сайте они и подписаны как
// «Пример обращения», без звёзд и без значка «проверено».
//
// Выдавать их за реальные отзывы нельзя: это вводит людей в заблуждение и
// является недостоверной рекламой (в РФ — ст. 5 ФЗ «О рекламе»).
// Когда появятся настоящие отзывы от клиентов, заменить этот файл на данные
// из базы и вернуть звёзды/верификацию.
//
