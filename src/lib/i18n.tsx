...
export const t: Dict = {
  ...
  cookieTitle: { ru: "Мы используем cookie", en: "We use cookies" },
  cookieText: { ru: "Мы используем cookie для работы сайта, аналитики и улучшения сервиса. Необходимые cookie обязательны, остальные применяются только с вашего согласия.", en: "We use cookies to run the site, for analytics and to improve the service. Essential cookies are required; others are used only with your consent." },
  cookieMore: { ru: "Подробнее в Политике конфиденциальности", en: "Learn more in the Privacy Policy" },
  cookieAccept: { ru: "Принять все", en: "Accept all" },
  cookieEssential: { ru: "Только необходимые", en: "Essential only" },
  ...
  priv1Text: { ru: "Оператором обработки персональных данных является владелец Платформы. Контактные данные оператора и ответственного за защиту данных (DPO) указаны в разделе «Контакты». По всем вопросам обработки данных вы можете обратиться по указанным реквизитам.", en: "The personal data controller is the owner of the Platform. The controller's contact details and those of the Data Protection Officer (DPO) are listed in the 'Contacts' section. You may contact us using these details regarding any data processing matter." },
  ...
  offer1Text: { ru: "Платформа предоставляет исполнителям платные услуги: размещение профиля, подписку, инструменты продвижения и доступ к заявкам. Перечень и стоимость услуг указаны в разделе «Тарифы». Оферта считается принятой в момент оплаты.", en: "The Platform provides providers with paid services: profile placement, subscription, promotion tools, and access to leads. The list and cost of services are specified in the 'Pricing' section. The offer is deemed accepted at the moment of payment." },
